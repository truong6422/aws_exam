...
from core.extra_fields import compat
...
class RangeField(DictField):
    ...
    def __init__(self, **kwargs):
        if compat.postgres_fields is None:
            assert False, ("'psycopg' is required to use {name}. Please install the 'psycopg2' (or 'psycopg' if you "
                           "are using Django>=4.2) library from 'pip'").format(
...
class IntegerRangeField(RangeField):
    ...
    default_child_attrs = {}
    range_type = compat.NumericRange
...
class DateTimeRangeField(RangeField):
    ...
    default_child_attrs = {}
    range_type = compat.DateTimeTZRange


class DateRangeField(RangeField):
    child_class = DateField
    default_child_attrs = {}
    range_type = compat.DateRange


if compat.postgres_fields:
    # monkey patch model-serializer to map Native django Range fields to
    # extra field's Range fields.
