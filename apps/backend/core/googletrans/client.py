...
from core.googletrans import urls, utils
from core.googletrans.constants import (
    DEFAULT_CLIENT_SERVICE_URLS,
    DEFAULT_FALLBACK_SERVICE_URLS,
    DEFAULT_USER_AGENT, LANGCODES, LANGUAGES, SPECIAL_CASES,
    DEFAULT_RAISE_EXCEPTION, DUMMY_DATA
)
from core.googletrans.gtoken import TokenAcquirer
from core.googletrans.models import Translated, Detected, TranslatedPart
...
class Translator:
    ...
    :param service_urls: Google Translate url list. URLs will be used randomly.
                         ...
                         To preferably use the non webapp api, service url should be translated.googleapis.com
    ...
    def _translate(self, text: str, dest: str, src: str):
        ...
        if r.status_code != 200 and self.raise_exception:
            raise BadRequest('Unexpected status code "{}" from {}'.format(r.status_code, self.service_urls))
        return r.text, r
    ...
    @staticmethod
    def _square_bracket_counts(square_bracket_counts, line):
        is_in_string = False
        ...
        return square_bracket_counts

    def _resp(self, data):
        square_bracket_counts = [0, 0]
        resp = ''
        for line in data.split('\n'):
            if f'"{RPC_ID}"' not in line[:30]:
                continue
            square_bracket_counts = self._square_bracket_counts(
                square_bracket_counts=square_bracket_counts,
                line=line
            )
            resp += line
    ...
    def translate(self, text: str, dest='en', src='auto'):
        dest, src = self._dest_src(dest, src)
        origin = text
        data, response = self._translate(text, dest, src)
        resp = self._resp(data=data)
    ...
    def translate_legacy(self, text, dest='en', src='auto', **kwargs):
        ...
        result = Translated(
            ...
            extra_data=extra_data, response=response, parts=None
        )
        return result
    ...
    def detect(self, text: str):
        ...
        result = Detected(
            ...
            response=translated.get_response()
        )
        return result
