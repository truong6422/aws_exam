...
from core.googletrans.utils import rshift
...
class TokenAcquirer:
    ...
    def _build_tkk(self, code):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign):
                ...
                if name in keys and isinstance(node.value, ast.Num):
                    keys[name] = node.value.n
                # the value can sometimes be negative
                elif name in keys and isinstance(node.value, ast.UnaryOp) and isinstance(node.value.op, ast.USub):
    ...
    def _update(self):
        ...
        if code:
            self.tkk = self._build_tkk(code=code)
        ...
        while g < size:
            ...
            if l < 2048:
                ...
                if (l & 64512) == 55296 and g + 1 < size and a[g + 1] & 64512 == 56320:
                    g += 1
                    l = 65536 + ((l & 1023) << 10) + (a[g] & 1023)  # noqa This bracket is important
                    e.append(l >> 18 | 240)
    ...
    def acquire(self, text):
        a = self._a(text)
        d = self.tkk.split('.') if self.tkk != '0' else ''
