...
class CoreConfig(AppConfig):
    name = 'core'
    label = 'django_core'  # avoid label conflict with apps.core (label="core")
    ...
    def ready(self):
        ...
        base_delete = QuerySet.delete

        def delete(queryset):
            objects = list(deepcopy(queryset).values())
            # get model label from queryset
            model = apps.get_model(queryset.model._meta.label)
            signals.pre_list_delete.send(sender=model, objects=objects)
            deleted, rows_count = base_delete(queryset)
            signals.post_list_delete.send(sender=model, objects=objects)
